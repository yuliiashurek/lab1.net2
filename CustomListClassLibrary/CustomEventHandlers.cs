...
public static class CustomEventHandlers
{
    ...
    {
        Console.WriteLine($"\t\t\t\t\t\t\t\t\tEvent: {e.ModificationTypes} Date: {e.DateTime}");
    }
        
    public static void PrintListResizedEventHandler(object sender, CustomListEventArgs e)
    {
        Console.WriteLine($"\t\t\t\t\t\t\t\t\tEvent: {e.ModificationTypes} Old capacity: {e.OldCapacity} New capacity: {e.NewCapacity} Date: {e.DateTime}");
    }
}